...
if (isset($_POST['update'])) {
	...
	while (!feof($fp)) {
		...
		if ($FieldValues[0] != '') {
			...
			$msg = new xmlrpcmsg("weberp.xmlrpc_InsertGLAccountSection", array(
				$accountsection,
				$user,
				$password
			));
	...
	prnMsg(_('Select a csv file containing the details of the account sections that you wish to import into webERP. ') . '<br />' . _('The first line must contain the field names that you wish to import. ') . '<a href ="Z_DescribeTable.php?table=accountsection">' . _('The field names can be found here') . '</a>', 'info');
