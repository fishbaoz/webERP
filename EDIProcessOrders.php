...
$sql = "SELECT id, segtag, maxoccur, seggroup FROM weberp_edi_orders_segs";
...
*/
				...
				switch ($NAD[0]){
					case 'IV': /* This Name and address detail is that of the party to be invoiced */
						...
						/*if NAD_C082[2] must = 9 then NAD_C082[0] is the EAN Intnat Article Numbering Assocn code of the customer - look up the customer by EDIReference*/
							$InvoiceeResult = DB_query("SELECT debtorno FROM weberp_debtorsmaster WHERE edireference='" . $NAD_C082[0] . "' AND ediorders=1");
					...
					case 'SN':
						...
						$BranchResult = DB_query("SELECT branchcode,
														...
														phoneno,
														email
												FROM weberp_custbranch INNER JOIN weberp_debtorsmaster ON weberp_custbranch.debtorno = weberp_custbranch.debtorno WHERE custbranchcode='" . $NAD_C082[0] . "' AND weberp_custbranch.debtorno='" . $Order->DebtorNo . "' AND weberp_debtorsmaster.ediorders=1");
	...
	if (mb_strlen($EmailText)>10){
		...
		if ($TryNextFile==True){ /*had to abort this message */
			...
			$Result = DB_query("SELECT realname, email FROM weberp_www_users WHERE fullaccess=7 AND email <>''");
