...
$Title = _('Uninstall a webERP plugin');

include('includes/header.inc');

echo '<p class="page_title_text noPrint" ><img src="' . $RootPath . '/css/' . $Theme . '/images/plugin.png" width="24px" title="' . _('Uninstall Plugin') . '" alt="" />' . _('Uninstall webERP Plugin') . '</p>';
	...
	foreach ($Scripts as $Script) {
		...
		while ($FileName = zip_read($ZipFile)) {
			...
			if ($Entry == $Script->name) {
				...
				prnMsg($Script->name . ' ' . _('has been successfully removed from webERP'), 'success');
			}
		}
		zip_close($ZipFile);
	}
