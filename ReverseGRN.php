...
if (!isset($_POST['SupplierID']) OR $_POST['SupplierID']==""){
	...
	$sql = "SELECT suppname FROM weberp_suppliers WHERE supplierid='" . $_SESSION['SupplierID'] . "'";
...
/* SQL to process the postings for the GRN reversal.. */
	...
	$SQL = "SELECT weberp_grns.podetailitem,
			...
			INNER JOIN weberp_locationusers ON weberp_locationusers.loccode=weberp_purchorders.intostocklocation AND weberp_locationusers.userid='" .  $_SESSION['UserID'] . "' AND weberp_locationusers.canupd=1
	...
	$SQL = "SELECT weberp_stockmaster.controlled
			FROM weberp_stockmaster WHERE stockid ='" . $GRN['itemcode'] . "'";
	...
	if ($ControlledRow[0]==1) { /*Then its a controlled item */
		...
		$SQL = "SELECT weberp_stockserialmoves.serialno,
				...
				WHERE weberp_stockmoves.stockid='" . $GRN['itemcode'] . "'
				AND weberp_stockmoves.type =25
				AND weberp_stockmoves.transno='" . $GRN['grnbatch'] . "'";
		...
		while ($SerialStockMoves = DB_fetch_array($GetStockMoveResult)){
			...
			$SQL = "SELECT weberp_stockserialitems.quantity
			        FROM weberp_stockserialitems
					WHERE weberp_stockserialitems.stockid='" . $GRN['itemcode'] . "'
					AND weberp_stockserialitems.loccode ='" . $GRN['intostocklocation'] . "'
					AND weberp_stockserialitems.serialno ='" . $SerialStockMoves['serialno'] . "'";
...
/*Now the SQL to do the update to the PurchOrderDetails */
	...
	$SQL = "UPDATE weberp_purchorderdetails
			...
			completed=0
			WHERE weberp_purchorderdetails.podetailitem = '" . $GRN['podetailitem'] . "'";
...
/*Need to update or delete the existing GRN item */
	...
	/* if this is not deleted then the purchorderdetail line cannot be deleted subsequentely */
		...
		$result = DB_query("DELETE FROM weberp_grns WHERE grnno='" . $_GET['GRNNo'] . "'",$ErrMsg,$DbgMsg,true);
	} else {
		$SQL = "UPDATE weberp_grns	SET qtyrecd = qtyrecd - " . $QtyToReverse . "
				WHERE weberp_grns.grnno='" . $_GET['GRNNo'] . "'";
	...
	/*If the GRN being reversed is an asset - reverse the weberp_fixedassettrans record */
	if ($GRN['assetid']!='0'){
		...
		/*now reverse the cost put to weberp_fixedassets */
		$SQL = "UPDATE weberp_fixedassets SET cost = cost - " . $GRN['stdcostunit'] * $QtyToReverse  . "
	...
	$SQL = "SELECT weberp_stockmaster.controlled
			...
			WHERE weberp_stockmaster.stockid = '" . $GRN['itemcode'] . "'";
...
/*end of Process Goods Received Reversal entry */
	...
	if (isset($_POST['ShowGRNS'])){
		...
		$sql = "SELECT grnno,
				...
				FROM weberp_grns
				INNER JOIN weberp_purchorderdetails ON weberp_purchorderdetails.podetailitem=weberp_grns.podetailitem
				INNER JOIN weberp_purchorders on weberp_purchorders.orderno = weberp_purchorderdetails.orderno
				INNER JOIN weberp_locationusers ON weberp_locationusers.loccode=weberp_purchorders.intostocklocation AND weberp_locationusers.userid='" .  $_SESSION['UserID'] . "' AND weberp_locationusers.canupd=1
				WHERE weberp_grns.supplierid = '" . $_POST['SupplierID'] . "'
				AND (weberp_grns.qtyrecd-weberp_grns.quantityinv) >0
				AND weberp_grns.deliverydate>='" . FormatDateForSQL($_POST['RecdAfterDate']) ."'";
