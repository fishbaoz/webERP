...
if ($_POST['DoUpgrade'] == _('Perform Upgrade')){
    ...
    if ($myrow[0]==0) {
        $sql='INSERT INTO `weberp_debtortype` ( `typeid` , `typename` ) VALUES (1, "Default")';
        $result=DB_query($sql);
        if (DB_error_no()==0) {
    ...
    $myrow=DB_fetch_array($result);
    if ($myrow[0]==0) {
        $sql='INSERT INTO `weberp_factorcompanies` ( `id` , `coyname` ) VALUES (null, "None")';
        $result=DB_query($sql);
        if (DB_error_no()==0) {
            echo '<td>' . _('Success') . '</td></tr>';
        } else {
    ...
    echo '<tr><td>' . _('Adding quotedate to weberp_salesorders table') . '</td>';
    $sql='DESCRIBE `weberp_salesorders` `quotedate`';
    $result=DB_query($sql);
    if (DB_num_rows($result)==0) {
        $sql='ALTER TABLE `weberp_salesorders` ADD `quotedate` date NOT NULL default "0000-00-00"';
        $result=DB_query($sql);
        if (DB_error_no()==0) {
            echo '<td>' . _('Success') . '</td></tr>';
        } else {
    ...
    echo '<tr><td>' . _('Adding confirmeddate to weberp_salesorders table') . '</td>';
    $sql='DESCRIBE `weberp_salesorders` `confirmeddate`';
    $result=DB_query($sql);
    if (DB_num_rows($result)==0) {
        $sql="ALTER TABLE `weberp_salesorders` ADD `confirmeddate` date NOT NULL default '0000-00-00'";
        $result=DB_query($sql);
        if (DB_error_no()==0) {
            echo '<td>' . _('Success') . '</td></tr>';
        } else {
