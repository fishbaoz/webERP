...
$Title = _('Install a webERP plugin');

include('includes/header.inc');

echo '<p class="page_title_text noPrint" ><img src="' . $RootPath . '/css/' . $Theme . '/images/plugin.png" width="24px" title="' . _('Install Plugin') . '" alt="" />' . _('Install webERP Plugin') . '</p>';
	...
	foreach ($Scripts as $Script) {
		...
		while ($FileName = zip_read($ZipFile)) {
			...
			if ($Entry == $Script->name) {
				...
				prnMsg($Script->name . ' ' . _('has been successfully added to webERP'), 'success');
			}
		}
		zip_close($ZipFile);
	}
