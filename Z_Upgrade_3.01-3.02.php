...
prnMsg(_('Upgrade script to number weberp_salesorderdetails records as required by version 3.02 .... please wait'),'info');

$TestAlreadyDoneResult = DB_query('SELECT * FROM weberp_salesorderdetails WHERE orderlineno>=1');
...
$SalesOrdersResult = DB_query('SELECT orderno, stkcode FROM weberp_salesorderdetails ORDER BY orderno');
...
while ($SalesOrderDetails = DB_fetch_array($SalesOrdersResult)) {
	...
	DB_query('UPDATE weberp_salesorderdetails
...
DB_query( 'ALTER TABLE weberp_salesorderdetails ADD CONSTRAINT salesorderdetails_pk primary key(orderno, orderlineno)');
