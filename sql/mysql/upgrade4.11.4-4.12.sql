...
ALTER TABLE `weberp_www_users` ADD `showdashboard` TINYINT(1) NOT NULL DEFAULT '0' COMMENT 'Display dashboard after login' AFTER `modulesallowed`;

INSERT INTO `weberp_scripts` (`script` ,`pagesecurity` ,`description` ) VALUES ('Z_UpdateItemCosts.php', '15', 'Use CSV of item codes and costs to update webERP item costs');
INSERT INTO `weberp_scripts` (`script` ,`pagesecurity` ,`description` ) VALUES ('CustomerBalancesMovement.php', '3', 'Allow customers to be listed in local currency with balances and activity over a date range');
INSERT INTO weberp_ `scripts` VALUES ('UserLocations.php',  '15',  'Location User Maintenance');
ALTER TABLE weberp_ `stockmoves` ADD  `userid` VARCHAR( 20 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER  `trandate`;

INSERT INTO weberp_ `securitytokens` VALUES ('16', 'QA');
...
INSERT INTO `weberp_config` (`confname` ,`confvalue` ) VALUES ('QualityLogSamples', '0');
INSERT INTO `weberp_config` (`confname` ,`confvalue` ) VALUES ('QualityCOAText', '');
INSERT INTO `weberp_config` (`confname` ,`confvalue` ) VALUES ('QualityProdSpecText', '');

--
-- Table structure for table `prodspecs`
--

CREATE TABLE weberp_IF NOT EXISTS `prodspecs` (
...
CREATE TABLE weberp_IF NOT EXISTS `qasamples` (
...
CREATE TABLE weberp_IF NOT EXISTS `qatests` (
...
CREATE TABLE weberp_IF NOT EXISTS `sampleresults` (
...
--
ALTER TABLE `weberp_stockdescriptiontranslations` CHANGE `descriptiontranslation` `descriptiontranslation` VARCHAR(50) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT 'Item''s short description';
-- Add field for the long description translation of an item:
ALTER TABLE `weberp_stockdescriptiontranslations` ADD `longdescriptiontranslation` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT 'Item''s long description';
-- Add a field to mark if a description needs revision:
ALTER TABLE `weberp_stockdescriptiontranslations` ADD `needsrevision` TINYINT(1) NOT NULL DEFAULT '0';
--
INSERT INTO `weberp_config` (`confname` ,`confvalue`) VALUES ('GoogleTranslatorAPIKey',  '');
...
INSERT INTO weberp_ scripts (`script`,`pagesecurity`,`description`) VALUES ('SalesTopCustomersInquiry.php',  '2',  'Shows the top customers sales for a selected date range');
